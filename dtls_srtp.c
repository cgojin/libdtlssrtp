...
SSL_CTX* dtls_ctx_init(
    ...
    SSL_CTX_set_verify(ctx,
                       (verify_mode & DTLS_VERIFY_FINGERPRINT) || (verify_mode & DTLS_VERIFY_CERTIFICATE) ? SSL_VERIFY_PEER | SSL_VERIFY_FAIL_IF_NO_PEER_CERT
                                                                                                          : SSL_VERIFY_NONE,
                       !(verify_mode & DTLS_VERIFY_CERTIFICATE) ? (cb ? cb : dtls_trivial_verify_callback) : NULL);
...
ptrdiff_t dtls_do_handshake(
     ...
     */
    if (sess->ssl == NULL || (dtls_sess_get_state(sess) != DTLS_CONSTATE_ACT && dtls_sess_get_state(sess) != DTLS_CONSTATE_ACTPASS)) {
        return 0;
    }
