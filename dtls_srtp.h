...
static inline const dsink* dtls_sess_get_sink(const dtls_sess* sess) { return sess->sink; }

static inline void dtls_sess_set_sink(dtls_sess* sess, const dsink* sink) { sess->sink = sink; }
 ...
 */
static inline X509* dtls_sess_get_pear_certificate(dtls_sess* sess) { return SSL_get_peer_certificate(sess->ssl); }
...
static inline void dtls_sess_set_state(dtls_sess* sess, enum dtls_con_state state) { sess->state = state; }
...
static inline BIO* dtls_sess_get_rbio(dtls_sess* sess) { return SSL_get_rbio(sess->ssl); }
static inline BIO* dtls_sess_get_wbio(dtls_sess* sess) { return SSL_get_wbio(sess->ssl); }
 ...
 */
static inline void srtp_key_material_extract(const srtp_key_material* km, srtp_key_ptrs* ptrs) {
...
static inline bool str_isempty(const char* str) { return ((str == NULL) || (str[0] == '\0')); }

static inline const char* str_nullforempty(const char* str) { return (str_isempty(str) ? NULL : str); }
